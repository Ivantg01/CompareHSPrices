...
import { GoogleCloudSkuFlat } from "../typesScrapper.ts";
...
const GoogleCloudPriceSchema = new Schema({
  searchCode: {
    type: String,
    sparse: true,
  }, /** internal own code to facilite searching */
...
GoogleCloudPriceSchema.index({ searchCode: 1 }, {
  unique: false,
  sparse: true,
});

export type GoogleCloudPriceModelType =
  & mongoose.Document
  & Omit<GoogleCloudSkuFlat, "id">;
export default mongoose.model<GoogleCloudPriceModelType>(
  "GoogleCloudPrice",
  GoogleCloudPriceSchema,
);
