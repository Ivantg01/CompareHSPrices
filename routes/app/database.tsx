...
export const handler: Handlers<FormData> = {
  async POST(req, ctx) {
    ...
    form.validateFormSelectsDatabase(formData, errors, values);
    values.servers = values?.highAvailability === "Y" ? "2" : "1";

    if (errors.length > 0) return ctx.render({ errors, values });
    //llamada a la API de busqueda de instancias
    try {
      //llama a /api/database con un POST y le envía los datos del formulario
      const response = await fetch(
        `${Deno.env.get("BACKEND_API_URL")}/api/database`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        throw new Error("Error buscando servicios de base de datos");
      }
...
export default function Page({ data }: PageProps<FormData>) {
  return (
    <div class="container mt-5">
      <h1 class="mb-4">
        Comparador de instancias de{" "}
        <span class="text-primary">Base de Datos</span>
      </h1>
      <h3>PostgresSQL en AWS, Azure y GCP y Oracle en OCI</h3>
      ...
      <form method="post" class="card shadow">
        <div class="card-body">
          ...
          <div className="row g-3">
            <FormField
              label="Edición de software Oracle en OCI"
              class="col-12 col-md-6 col-lg-4 col-xl-4 mb-2"
              name="softwareEdition"
              type="select"
              options={form.DB_SW_EDITION}
              value={data?.values?.softwareEdition ||
                form.DB_SW_EDITION[0].value}
            />

            <FormField
              label="Licencia Oracle propia en OCI"
              class="col-12 col-md-6 col-lg-4 col-xl-4 mb-2"
              name="byol"
              type="select"
              options={form.YES_NO}
              value={data?.values?.byol || form.YES_NO[0].value}
            />
            ...
            <FormField
              ...
              type="select"
              options={form.YES_NO}
              value={data?.values?.highAvailability || form.YES_NO[0].value}
            />
          </div>
        </div>
